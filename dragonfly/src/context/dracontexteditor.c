...
DraControlLeftRight dra_context_editor_calculate_control_left_right(DraContextEditor *context_editor, ChaLineWo *line, ChaCursorWo *cursor) {
	DraContextEditorClass *ce_class = DRA_CONTEXT_EDITOR_GET_CLASS(context_editor);
	DraControlLeftRight result;
	result.left = -1;
	result.right = -1;
	if (ce_class->calculateControlLeftRight) {
		result = ce_class->calculateControlLeftRight(context_editor, line, cursor);
	}
	return result;
}
