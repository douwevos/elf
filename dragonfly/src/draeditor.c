...
static void l_control_location(ChaEditor *editor, ChaCursorWo *cursor, gboolean clicked);
...
static void dra_editor_class_init(DraEditorClass *clazz) {
	...
	cha_clazz->controlLocation = l_control_location;
...
void dra_editor_request_mark_occurrences(DraEditor *editor) {
	...
	if (cha_preferences_wo_get_mark_occurrences(prefs)) {
		...
		if (document_view && DRA_IS_DOCUMENT_VIEW(document_view)) {
...
void dra_editor_set_occurrence_text(DraEditor *editor, CatStringWo *text) {
	...
	if (!cat_string_wo_equal(text, priv->occurrence_text)) {
		...
		DraDocumentView *document_view = (DraDocumentView *) cha_editor_get_document_view((ChaEditor *) editor);
		if (DRA_IS_DOCUMENT_VIEW(document_view)) {
...
CatStringWo *dra_editor_get_line_info_key(DraEditor *editor) {
	DraEditorPrivate *priv = dra_editor_get_instance_private(editor);
	return priv->slot_key;
}


void dra_editor_open_goto_line_dialog(DraEditor *editor) {
	...
	if (result == GTK_RESPONSE_OK) {
		...
		if (line_nr>=1) {
			cha_editor_goto_line((ChaEditor *) editor, line_nr-1);
		}
...
static void l_marker_over(ChaEditor *editor, ChaLineLocationWo *location, long long y_marker_view) {
	...
	if (priv->tag_current_line_location!=priv->tag_line_location) {
		...
		priv->tag_timeout_tag = gdk_threads_add_timeout_full(0, 200, (GSourceFunc) l_timeout, g_object_ref(editor), g_object_unref);
	}
...
static void l_control_location(ChaEditor *editor, ChaCursorWo *cursor, gboolean clicked) {
	...
	DraDocumentView *document_view = (DraDocumentView *) cha_editor_get_document_view((ChaEditor *) editor);
	cat_log_debug("document_view=%p, cursor=%O", document_view, cursor);
	if (document_view==NULL || !DRA_IS_DOCUMENT_VIEW(document_view)) {
		return;
	}

	gboolean invalidate = TRUE;
	ChaRevisionWo *revision_ref = NULL;
	ChaPageWo *page = NULL;
	ChaLineWo *line = NULL;

	int left = -1;
	int right = -1;
	if ((cursor != NULL) && (priv->context_editor!=NULL)) {
		ChaDocument *document = cha_editor_get_document(editor);
		revision_ref = cha_document_get_current_revision_ref(document);
		ChaLineLocationWo *ll = cha_cursor_wo_get_line_location(cursor);

		page = cha_revision_wo_page_at(revision_ref, cha_line_location_wo_get_page_index(ll));
		cha_page_wo_hold_lines(page);
		line = cha_page_wo_line_at(page, cha_line_location_wo_get_page_line_index(ll));
		CatStringWo *text = cha_line_wo_get_text(line);
		DraControlLeftRight left_right = dra_context_editor_calculate_control_left_right(priv->context_editor, line, cursor);
		left = left_right.left;
		right = left_right.right;
		cha_editor_show_cursor_hand(editor);
	} else {
		cha_editor_show_cursor_text(editor);
	}
	cat_log_debug("left=%d, right=%d, line=%O", left, right, line);

	dra_document_view_set_control_left_right(document_view, line, left, right);
			...
			if (line_info) {
				...
				if (line_tags) {
					...
					while(cat_iiterator_has_next(iter)) {
						...
						if (dra_line_tag_wo_get_tag_type(tag)==DRA_TAG_TYPE_SPELL) {
							int start;
							int end;
...
static gboolean l_key_press_event(GtkWidget *gwidget, GdkEventKey *eev, gpointer data) {
	...
	if (cha_editor_im_filter_keypress((ChaEditor *) editor, (GdkEvent *) eev)) {
	...
	switch (key_val) {
		...
		case GDK_KEY_Right: {
			if (!shift_pressed && alt_pressed) {
				uow = (ChaUow *) cha_uow_history_move_new(TRUE);
			} else {
		...
		case GDK_KEY_Left: {
			if (!shift_pressed && alt_pressed) {
				uow = (ChaUow *) cha_uow_history_move_new(FALSE);
			} else {
		...
		case GDK_KEY_Return: {
			...
			ChaEditorClass *editor_class = CHA_EDITOR_GET_CLASS(editor);
			CatStringWo *text = cat_string_wo_new_with(eev->string);
			if (edit_mode == CHA_EDIT_MODE_OVERWRITE) {
				edit_mode = CHA_EDIT_MODE_INSERT;
			}
			uow = editor_class->createInsertOrReplaceUow(editor, edit_mode, text);
			cat_unref_ptr(text);
		} break;
	...
	if (uow==NULL && !ctrl_pressed && !mod1_pressed) {
		...
		cat_log_debug("no_text_input_mask=%d, eev->state=%d, eev->type=%d", no_text_input_mask, eev->state, eev->type);

		if (eev->type == GDK_KEY_PRESS && (eev->state & no_text_input_mask) == 0) {
			gunichar ch = gdk_keyval_to_unicode(eev->keyval);
			cat_log_debug("ch=%d", ch);
			CatStringWo *insert_string = NULL;
			if (ch != 0 && (!g_unichar_iscntrl (ch) || ch=='\t' )) {
				...
				insert_string = cat_string_wo_new_with(buf);
			} else if (eev->string[0] != 0) {
				insert_string = cat_string_wo_new_with(eev->string);
			}

			cat_log_debug("insert_string=%O", insert_string);
...
static gboolean l_key_release_event(GtkWidget *gwidget, GdkEventKey *eev, gpointer data) {
	...
	if (eev->keyval == GDK_KEY_Control_L || eev->keyval == GDK_KEY_Control_R) {
		ChaEditorClass *editor_class = CHA_EDITOR_GET_CLASS((ChaEditor *) editor);
		if (editor_class->controlLocation) {
			editor_class->controlLocation(editor, NULL, FALSE);
		}
	}
...
DraAcContext *dra_editor_create_auto_complete_context(DraEditor *editor) {
	cat_log_info("complete_context for:%p", editor);
	DraAcContext *result = dra_ac_context_new((ChaEditor *) editor);
	cat_log_info("result:%p", result);
	cat_log_info("complete_context for:%O", editor);
	DraEditorPanel *editor_panel = dra_editor_get_panel(editor);
	cat_log_info("editor_panel:%p", editor_panel);
	DraPanelOwner *panel_owner = (DraPanelOwner *) lea_panel_get_panel_owner((LeaPanel *) editor_panel);
