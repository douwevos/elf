...
DraLineTagWo *dra_line_tag_wo_new(long long row, DraTagType tag_type, DraTagLevel tag_level) {
...
DraTagLevel dra_line_tag_wo_get_tag_level(DraLineTagWo *line_tag) {
	DraLineTagWoPrivate *priv = dra_line_tag_wo_get_instance_private(line_tag);
	return priv->tag_level;
}

void dra_line_tag_wo_get_color(DraLineTagWo *line_tag, double *red, double *green, double *blue) {
	DraLineTagWoPrivate *priv = dra_line_tag_wo_get_instance_private(line_tag);

	switch(priv->tag_level) {
		case DRA_TAG_LEVEL_ERROR : {
			*red = 1.0;
			*green = 0.0;
			*blue = 0.0;
		} break;
		case DRA_TAG_LEVEL_WARN : {
			*red = 1.0;
			*green = 0.55;
			*blue = 0.0;
		} break;
		case DRA_TAG_LEVEL_UNSAFE : {
			*red = 0.9;
			*green = 0.9;
			*blue = 0.0;
		} break;
		case DRA_TAG_LEVEL_INFO : {
			*red = 0.6;
			*green = 0.6;
			*blue = 0.8;
		} break;
		case DRA_TAG_LEVEL_MARK : {
			*red = 0.7;
			*green = 0.7;
			*blue = 0.7;
		} break;
...
static CatWo *l_construct_editable(CatWo *e_uninitialized, CatWo *original, struct CatWoInfo *info) {
	...
	if (original) {
		...
		priv->tag_level = rpriv->tag_level;
	} else {
		priv->row = -1;
		priv->start_index = -1;
		priv->end_index = -1;
		...
		priv->tag_type = DRA_TAG_TYPE_PARSER;
		priv->tag_level = DRA_TAG_LEVEL_ERROR;
	}
...
static gboolean l_equal(const CatWo *wo_a, const CatWo *wo_b) {
	...
	return (priv_a->row==priv_b->row)
				...
				&& (priv_a->tag_level==priv_b->tag_level)
...
static CatWo *l_clone_content(CatWo *e_uninitialized, const CatWo *wo_source) {
	...
	if (wo_source) {
		...
		priv->tag_level = priv_src->tag_level;
