...
struct _DraDocumentViewPrivate {
	...
	DraLineLayout *control_line_layout;
...
static void l_dispose(GObject *object) {
	...
	cat_unref_ptr(priv->control_line_layout);
...
void dra_document_view_construct(DraDocumentView *document_view, ChaDocument *document, PangoContext *pango_context, GtkWidget *widget, CatStringWo *a_line_info_key) {
...
DraDocumentView *dra_document_view_new(ChaDocument *document, PangoContext *pango_context, GtkWidget *widget, CatStringWo *a_line_info_key) {
	...
	dra_document_view_construct(result, document, pango_context, widget, a_line_info_key);
	return result;
}
...
void dra_document_view_set_control_left_right(DraDocumentView *document_view, ChaLineWo *line, int left, int right) {
	DraDocumentViewPrivate *priv = dra_document_view_get_instance_private(document_view);
	gboolean invalidate = TRUE;

	DraLineLayout *line_layout = line==NULL ? NULL : (DraLineLayout *) cha_document_view_get_line_layout_ref(document_view, line);
	if ((line_layout!=priv->control_line_layout) && (priv->control_line_layout!=NULL)) {
		cha_line_layout_lock(priv->control_line_layout, TRUE);
		dra_line_layout_set_control_left_right(priv->control_line_layout, -1, -1);
		cha_line_layout_unlock(priv->control_line_layout);
		invalidate = TRUE;
	}
	if (line_layout) {
		cha_line_layout_lock(line_layout, TRUE);
		dra_line_layout_set_control_left_right(line_layout, left, right);
		cha_line_layout_unlock(line_layout);
	}
	cat_ref_swap(priv->control_line_layout, line_layout);
}
