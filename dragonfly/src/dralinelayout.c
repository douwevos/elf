...
struct _DraLineLayoutPrivate {
	...
	int control_left, control_right;
};
...
void dra_line_layout_set_control_left_right(DraLineLayout *line_layout, int left, int right) {
	DraLineLayoutPrivate *priv = dra_line_layout_get_instance_private(line_layout);
	cat_log_debug("conte_left_right=%d, %d", left, right);
	if ((priv->control_left==left) && (priv->control_right==right)) {
		return;
	}
	priv->control_left = left;
	priv->control_right = right;
	cha_line_layout_mark_for_repaint((ChaLineLayout *) line_layout);
	cha_line_layout_mark_attributes_dirty((ChaLineLayout *) line_layout);
}
...
static void l_apply_extra_attributes(ChaLineLayout *line_layout, PangoAttrList *pango_attr_list, ChaPrefsColorMapWo *color_map) {
	...
	if (priv->control_left>=0 && priv->control_right>=0) {
		cat_log_debug("applying:%d, %d", priv->control_left, priv->control_right);
		PangoAttribute *ptr = pango_attr_underline_color_new(0x4444,0x4444,0xCCCC);
//		PangoAttribute *ptr = pango_attr_background_new(0x0000, 0xE8E8, 0xF8F8);
		ptr->start_index = priv->control_left;
		ptr->end_index = priv->control_right;
		pango_attr_list_change(pango_attr_list, ptr);
		PangoAttribute *ptr2 = pango_attr_underline_new(PANGO_UNDERLINE_SINGLE);
		ptr2->start_index = priv->control_left;
		ptr2->end_index = priv->control_right;
		pango_attr_list_change(pango_attr_list, ptr2);
	}
}
