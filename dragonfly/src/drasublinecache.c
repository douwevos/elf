...
static gboolean l_update(ChaSubLineCache *sub_line_cache, ChaSubLineCacheUpdate *update_ctx) {
	...
	if (update_ctx->cache_dirty) {
		...
		if (a_line_tags) {
			...
			DraLineTagWo *best_line_tag_wo = NULL;
			while(cat_iiterator_has_next(iter)) {
				...
				if (best_line_tag_wo == NULL) {
					best_line_tag_wo = line_tag_wo;
				} else {
					DraTagType this_type = dra_line_tag_wo_get_tag_type(line_tag_wo);
					DraTagType best_type = dra_line_tag_wo_get_tag_type(best_line_tag_wo);
					if (this_type < best_type) {
						best_line_tag_wo = line_tag_wo;
					} else if (this_type == best_type) {
						DraTagLevel this_level = dra_line_tag_wo_get_tag_level(line_tag_wo);
						DraTagLevel best_level = dra_line_tag_wo_get_tag_level(best_line_tag_wo);
						if (this_level < best_level) {
							best_line_tag_wo = line_tag_wo;
						}
					}
				}
			...
			if (!is_printer_mode && best_line_tag_wo) {
				cairo_save(surface_cr);
				...
				double red, green,blue;
				dra_line_tag_wo_get_color(best_line_tag_wo, &red, &green, &blue);
				cairo_set_source_rgb(surface_cr, red, green, blue);
