...
struct _ChaDocumentPrivate {
	...
	CatLinkedList *e_cursor_history;
	int history_index;
	int cursor_history_index;
...
static void l_dispose(GObject *object) {
	...
	cat_unref_ptr(priv->e_cursor_history);
...
void cha_document_construct(ChaDocument *document, struct _ChaDocumentManager *document_manager) {
	...
	priv->e_cursor_history = cat_linked_list_new();
...
void cha_document_anchor_document_full(ChaDocument *document, ChaDocumentAnchorMode anchor_mode) {
	...
	if (priv->e_revision) {
		...
		gboolean has_new_content = FALSE;
...
//			}
		...
		if (has_new_content) {
			cat_linked_list_clear(priv->e_cursor_history);
			priv->cursor_history_index = -1;
		}
		cat_linked_list_append(priv->e_cursor_history, a_new_rev);
		priv->cursor_history_index++;
...
void cha_document_undo(ChaDocument *document) {
	...
	if (priv->history_index>0) {
		...
		ChaRevisionWo *a_rev = (ChaRevisionWo *) cat_atomic_reference_get_unsafe(priv->a_revision_ref);
		cat_linked_list_append(priv->e_cursor_history, (GObject *) a_rev);
		priv->cursor_history_index++;
...
void cha_document_history_back(ChaDocument *document) {
	ChaDocumentPrivate *priv = cha_document_get_instance_private(document);
	if (priv->cursor_history_index>0) {
		priv->cursor_history_index--;
		cat_log_error("priv->cursor_history_index=%d", priv->cursor_history_index);
		cat_log_error("cursor_history.size=%d", cat_linked_list_size(priv->e_cursor_history));
		ChaRevisionWo *a_new_rev = (ChaRevisionWo *) cat_linked_list_get(priv->e_cursor_history, priv->cursor_history_index);
...
void cha_document_history_forward(ChaDocument *document) {
	ChaDocumentPrivate *priv = cha_document_get_instance_private(document);
	int hist_size = cat_linked_list_size(priv->e_cursor_history);
	if (priv->cursor_history_index<hist_size-1) {
		priv->cursor_history_index++;
		ChaRevisionWo *a_new_rev = (ChaRevisionWo *) cat_linked_list_get(priv->e_cursor_history, priv->cursor_history_index);
