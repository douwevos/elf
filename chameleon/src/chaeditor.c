...
static ChaUow *l_create_insert_or_replace_uow(ChaEditor *editor, CatStringWo *text);
...
static void cha_editor_class_init(ChaEditorClass *clazz) {
	...
	clazz->controlLocation = NULL;
	clazz->createInsertOrReplaceUow = l_create_insert_or_replace_uow;
...
void cha_editor_show_cursor_text(ChaEditor *editor) {
	ChaEditorPrivate *priv = cha_editor_get_instance_private(editor);
	if (priv->cursor) {
		gdk_window_set_cursor(gtk_widget_get_window((GtkWidget *) editor), priv->cursor);
	}
}

void cha_editor_show_cursor_hand(ChaEditor *editor) {
	ChaEditorPrivate *priv = cha_editor_get_instance_private(editor);
	if (priv->cursor_hand) {
		gdk_window_set_cursor(gtk_widget_get_window((GtkWidget *) editor), priv->cursor_hand);
	}
}
...
void cha_editor_set_preferences(ChaEditor *editor, ChaPreferencesWo *a_prefs) {
	ChaEditorPrivate *priv = cha_editor_get_instance_private(editor);
	cat_log_debug("%p, a_prefs=%O", editor, a_prefs);
...
gboolean cha_editor_goto_line(ChaEditor *editor, long long row) {
...
static ChaUow *l_create_insert_or_replace_uow(ChaEditor *editor, CatStringWo *text) {
	...
	if (edit_mode==CHA_EDIT_MODE_INSERT) {
		uow = (ChaUow *) cha_uow_insert_text_new(cat_ref_ptr(text));
	} else {
		uow = (ChaUow *) cha_uow_replace_text_new(cat_ref_ptr(text));
	}
	return uow;
...
static void l_im_context_commit_cb(GtkIMContext *context, const gchar *str, ChaEditor *editor) {
	...
	CatStringWo *text = cat_string_wo_new_with(str);
	ChaUow *uow = editor_class->createInsertOrReplaceUow(editor, mode, text);
	cat_unref_ptr(text);
...
static gboolean l_button_release_event_cb(GtkWidget *gwidget, GdkEventButton *eev, gpointer data) {
	...
	if (editor_class->controlLocation && ctrl_pressed) {
		ChaDocument *document = cha_document_view_get_document(priv->document_view);
		gint xmouse = eev->x;
		gint ymouse = eev->y;
		ChaCursorWo *cursor = cha_document_view_cursor_at_xy(priv->document_view, xmouse, ymouse);
		editor_class->controlLocation(editor, cursor, TRUE);
		cat_unref_ptr(cursor);
	}
...
//		gboolean mod5_pressed = eev->state & GDK_MOD5_MASK;
		...
		if (editor_class->controlLocation) {
			ChaDocument *document = cha_document_view_get_document(priv->document_view);
			ChaCursorWo *cursor = cha_document_view_cursor_at_xy(priv->document_view, xmouse, ymouse);
			editor_class->controlLocation(editor, cursor, FALSE);
			cat_unref_ptr(cursor);
		}
	}
	return FALSE;
...
static void l_widget_realize(GtkWidget *widget) {
	...
	if (priv->document_view==NULL) {
		...
		cat_log_error("priv->document_view=%p", priv->document_view);
	...
	if (priv->cursor_hand==NULL) {
		priv->cursor_hand = gdk_cursor_new_for_display(gdk_window_get_display(window), GDK_HAND2);
	}
...
static void l_toggle_preferences_flag(ChaEditor *editor, ApplyTogglePreferencessFlag apply_cb) {
	ChaEditorPrivate *priv = cha_editor_get_instance_private(editor);
	cat_log_error("%p, a_prefs=%O", editor, priv->a_preferences);
