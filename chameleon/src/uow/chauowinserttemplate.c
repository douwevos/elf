...
static void l_dispose(GObject *object) {
	...
	cat_unref_ptr(priv->template_text);
...
ChaUowInsertTemplate *cha_uow_insert_template_new(CatStringWo *template_label) {
	...
	priv->template_text = NULL;
	return result;
}

ChaUowInsertTemplate *cha_uow_insert_template_new_raw(CatStringWo *template_text) {
	ChaUowInsertTemplate *result = g_object_new(CHA_TYPE_UOW_INSERT_TEMPLATE, NULL);
	cat_ref_anounce(result);
	ChaUowInsertTemplatePrivate *priv = cha_uow_insert_template_get_instance_private(result);
	cha_uow_construct((ChaUow *) result, FALSE);
	priv->template_label = NULL;
	priv->template_text = cat_ref_ptr(template_text);
	return result;
}
...
static void l_uow_run(ChaUow *self, struct _ChaEditor *editor, ChaDocumentView *document_view) {
	...
	CatStringWo *template_text = NULL;
	if (priv->template_text) {
		template_text = priv->template_text;
	} else {
		...
		template_text = cha_prefs_template_wo_get_template(a_template);
	}
	...
	if (cha_form_wo_field_count(e_form)>0) {
		ChaFormFieldWo *first_field = cha_form_wo_get_at(e_form, 0);
		ChaCursorMWo *start = cha_form_field_wo_get_start(first_field);
		ChaCursorMWo *end = cha_form_field_wo_get_end(first_field);
		ChaLineLocationWo *ll = cha_revision_wo_calculate_line_location(e_revision, cha_cursorm_wo_get_row(start));
		ChaCursorWo *start_cur = cha_cursor_wo_new_ll_offset(ll, cha_cursorm_wo_get_column(start));

		ChaLineLocationWo *llend = cha_revision_wo_calculate_line_location(e_revision, cha_cursorm_wo_get_row(end));
		ChaCursorWo *end_cur = cha_cursor_wo_new_ll_offset(llend, cha_cursorm_wo_get_column(end));

		cha_document_view_set_plain_selection(document_view, start_cur, end_cur);
		cha_revision_wo_set_cursor(e_revision, end_cur);
	}
