...
struct _ChaUowInsertTextPrivate {
	CatStringWo *a_text;
	gboolean delta_cursor_set;
	int delta_cursor_row, delta_cursor_column;
};
...
ChaUowInsertText *cha_uow_insert_text_new(CatStringWo *ca_text) {
	...
	priv->a_text = ca_text;
	priv->delta_cursor_set = FALSE;
	return result;
}

void cha_uow_insert_text_set_cursor_delta(ChaUowInsertText *uow, int delta_row, int delta_column) {
	ChaUowInsertTextPrivate *priv = cha_uow_insert_text_get_instance_private(uow);
	priv->delta_cursor_set = TRUE;
	priv->delta_cursor_row = delta_row;
	priv->delta_cursor_column = delta_column;
}
...
static void l_uow_run(ChaUow *self, struct _ChaEditor *editor, ChaDocumentView *document_view) {
	...
	ChaCursorMWo *cursorm = NULL;
	if (priv->delta_cursor_set) {
		ChaCursorWo *cursor = cha_revision_wo_get_cursor(e_revision);
		cursorm = cha_revision_wo_to_cursorm(e_revision, cursor);
		cha_cursorm_wo_move(cursorm, priv->delta_cursor_row, priv->delta_cursor_column);
		if (priv->delta_cursor_row>0) {
			cha_cursorm_wo_set_column(cursorm, priv->delta_cursor_column);
		}
	}

	cha_revision_wo_get_editable_cursor(e_revision);
	...
	if (cursorm) {
		ChaLineLocationWo *ll = cha_revision_wo_calculate_line_location(e_revision, cha_cursorm_wo_get_row(cursorm));
		cha_cursor_wo_set_line_location(e_cursor, ll);
		cha_cursor_wo_set_x_cursor_bytes(e_cursor, cha_cursorm_wo_get_column(cursorm));
	}
